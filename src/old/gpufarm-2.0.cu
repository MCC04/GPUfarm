...
int main(int argc, char **argv){
    ...
    const int streamSize = N_size / (K_exec);//*2 ;
    const int streamBytes = streamSize* sizeof(float) ;
    GRID=streamSize/BLOCK;    
    clocks=new int[GRID*K_exec]; 
    ...
    cudaMalloc(&clocks_d, K_exec*GRID*sizeof(int));
    ...
    memset(clocks, 0, K_exec*GRID*sizeof(int));
    ...
    for (int r = 0; r < K_exec; ++r) {  
        ...
        for (int i = 0; i < K_exec; ++i) {
            ...
            #ifdef STRIDE
                cosGridStride<<<GRID, BLOCK, 0, stream[i]>>>(M_iter, N_size, x_d, offset, clocks_d);
            #else
                cosKernel<<<GRID, BLOCK, 0, stream[i]>>>(M_iter, N_size, x_d, offset, clocks_d);
            #endif
            cudaMemcpyAsync( &cosx[offset], &x_d[offset], streamBytes, cudaMemcpyDeviceToHost, stream[i]);
            cudaMemcpyAsync( &clocks[clockOffs], &clocks_d[clockOffs],GRID*sizeof(int), cudaMemcpyDeviceToHost, stream[i]);
