...
void easyRandomMatrix(const int m, int n,float *mat, float var)
{
    ...
    for(int r=0; r<m; ++r)
        for(int c=0; c<n; ++c){
            //int rnd = (float)std::rand();
            //float val = LOW + (rnd*(HIGH-LOW)/RAND_MAX);
            setMatrixVal(mat, r, c, n, (float)(r+c*3));
            count++;
        }     
}
                                ...
                                int numRows, int numCols, const float* filter, const int filterWidth)
{
    ...
    */
    ...
    if (r < numCols && c < numRows){
        ...
        for (int i = 0; i < filterWidth; ++i) {
            for (int j = 0; j < filterWidth; ++j) {
                int p_x = r+i-filterWidth/2;
                int p_y = c+j-filterWidth/2;
                p_x = min(max(p_x,0), numRows-1);
                p_y = min(max(p_y,0), numCols-1);
                float filter_value = filter[ i*filterWidth+j ];
                blur += filter_value*static_cast<float>(inputChannel[ p_x*numCols+p_y ]);
...
{          
    ...
    #else
        dim3 gridDims((unsigned int)((bytesSize+BLOCK-1)/blockDims.x), 1, 1 );
...
//    int bytes=kerdim*kerdim*sizeof(unsigned char);
    ...
    //gpuErrchk(cudaMallocManaged(&ker, bytes));
    getGaussian(ker, kerdim, sigma);        

    dim3 blockDims( BLOCK,BLOCK,1 );
    gpuErrchk( cudaMemcpyAsync(in_d, img_in, bytesSize, cudaMemcpyHostToDevice, strm) );    
    gpuErrchk( cudaMemcpyAsync(ker_d, ker, kerBytes, cudaMemcpyHostToDevice, strm) );    
    //#ifdef LOWPAR        
      //  dim3 gridDims( 1,1,1 );
                      dim3 gridDims(((width*3)+blockDims.x-1)/blockDims.x, ((height*3)+blockDims.y-1)/blockDims.y, 1 ); //dim3 gridDims((unsigned int) ceil((double)(bytes/blockDims.x)), 1, 1 );
   ...
   /* #else
        ...
        gaussianBlurKer<<<gridDims, blockDims, 0, strm>>>(in_d, out_d, height, width, ker, kerdim);
    #endif*/
    gpuErrchk( cudaMemcpyAsync( img_out, out_d, bytesSize, cudaMemcpyDeviceToHost, strm) );
    cudaDeviceSynchronize();
    cudaFree(ker_d);
