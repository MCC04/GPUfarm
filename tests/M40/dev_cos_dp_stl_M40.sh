...
do	
	...
	do
		...
		echo -ne "$nTests \n"			
		nvprof --log-file ./profiling/dev_datapar$N-$m-$B.txt ./bin/datapar.out $GPU $B $N $m 0 0 >> ./results/dev_cos_dp_stl.txt
...
do			
	...
	do
		...
		echo -ne "$nTests \n"			
		nvprof --log-file ./profiling/dev_str_low$N-$m-$B-3.txt ./bin/streamlow.out $GPU $B $N $m 1 3 >> ./results/dev_cos_dp_stl.txt
...
do			
	...
	do
		...
		echo -ne "$nTests \n"			
		nvprof --log-file ./profiling/dev_str_low$N-$m-$B-cores.txt ./bin/streamlow.out $GPU $B $N $m 1 0 >> ./results/dev_cos_dp_stl.txt
