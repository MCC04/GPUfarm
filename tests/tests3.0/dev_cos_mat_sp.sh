...
do
	...
	do
		...
		do
			echo running with elements N = $N , iterations M = $m
			nvprof --log-file ./profiling/dev_str_sp$N-$m.txt ./bin/stream.out $GPU $BLOCK $K_ex $m $N \
								>> ./results/dev_sp.txt
		done
	done
done
...
do
	...
	do		
		...
		do
			echo running with elements N = $N , iterations M = $m
			nvprof --log-file ./profiling/dev_str_sp_low$N-$m.txt ./bin/streamlow.out $GPU $BLOCK $K_ex $m $N \
								>> ./results/dev_sp.txt
		done
	done
done
	...
	do
		...
		do		
			...
			nvprof --log-file ./profiling/dev_smm_sp$N-$n.txt \ 
								...
								>> ./results/dev_sp.txt #chunk = Nmats/Nmats = 1
		done
	done
done
	...
	do
		...
		do		
			...
			nvprof --log-file ./profiling/dev_smm_low_sp$N-$n.txt \ 
								...
								>> ./results/dev_sp.txt #chunk = N/N = 1
		done
	done
done
...
#								./bin/matmul.out $GPU $BLOCK $n 1 $N $n \ #chunk = N/N = 1
#								>> ./results/dev_sp.txt
#		done
#	done
#done
...
#								./bin/matmullow.out $GPU $BLOCK $n 1 $N $n \ #chunk = N/N = 1
#								>> ./results/dev_sp.txt
#		done
#	done
#done